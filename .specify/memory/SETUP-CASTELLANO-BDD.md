...
│   └── esta-archivo.md           # Guía de configuración
│
├── templates/
│   ├── overrides/                # ⭐ TEMPLATES EN CASTELLANO + BDD
...
│   │
│   └── [otros templates core]    # Templates originales (backup)
│
├── integrations/
...
1. **`.specify/templates/overrides/`** ← **Tus overrides personalizados (castellano)**
2. `.specify/presets/templates/` ← Presets (si aplican)
3. `.specify/extensions/templates/` ← Extensiones
4. `.specify/templates/` ← Core de Spec Kit (inglés)

**Resultado**: Tus templates en castellano se usan automáticamente.
...
- [x] Templates en `.specify/templates/overrides/` creados (spec, plan, tasks, checklist)
- [x] Todos los templates en **castellano**
- [x] Templates incluyen **EARS** para requisitos
- [x] Templates incluyen **Gherkin** para escenarios
- [x] Constitución del proyecto aprobada (`.specify/memory/constitution.md`)
